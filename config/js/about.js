function whenDOMReady() {
    ...
    if (location.pathname.startsWith('/about/')) meuicat.Introduction(), meuicat.runtimen(), meuicat.fiftyonela();
}
...
var meuicat = {
	...
	runtimen: function() {
		let t = new Date("2024/04/23 00:00:00")
			.getTime(),
			n = new Date()
			.getTime(),
		...
		c && (c.innerHTML = `已稳定运行 ${l} 光年 🚀`),
			setTimeout(meuicat.runtime, 1e3);
	}, // about 运行时间
	fiftyonela: function() {
		fetch('https://v6-widget.51.la/v6/{3JyEQIFLfwnu55rQ}/quote.js')
			.then(res => res.text())
			.then((data) => {
